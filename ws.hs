...
data StackElement = SDInt Int | SDDouble Double | SDString String | SDBool Bool |
    ...
    Syntax String | Ignorable String -- syntax types
    deriving Eq
...
instance Show StackElement where
    ...
    show (Ignorable _) = error "Ignorable should have been ignored by tokenizer!"
...
tokenizeHelper input tokens =
    let (token, match) = tryParse input tokenParsers   -- parse token
        newInput = rmPrefix match input   -- remove matched string from input

    -- recursively tokenize, if its a whitespace or comma (Ignorable) ignore the token!
    in case token of
        Ignorable _ -> tokenizeHelper newInput tokens
...
tokenParsers :: [(String, String -> StackElement)]
tokenParsers = [
...
-- recall the lambda tokens have been reversed, so the based case is '| Int {', which is normally '{ Int |'
...
bPow = dispatchNumBinFunc (^) powerDouble
...
-- retrieves all the variabels and extracts the numbers from them, calls 'nub' to get the unqiue ones,
-- and them sorts them with the largest at the left most position. Then maps each number back top a var! 
distinctVariables :: [StackElement] -> [StackElement]
distinctVariables tokens = 
    map (\e -> SDVar ("x" ++ show e))
        (sortBy (comparing Down)
                (nub [getVarNumber varName | SDVar varName <- tokens]))
